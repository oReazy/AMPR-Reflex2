...
async def addNewAccount(username, password):  # Создание нового аккаунта в базе данных
    try:
        ...
        async with connection.cursor() as cursor:
            new_user = "INSERT INTO `users` (password, hash, nick, lvl, exp, dollars, euro, yen, pounds, bank_dollars, bank_euro, bank_yen, bank_pounds, donate, isStaff, inventory, state, avatar, sex, age, passport, skills, clothes, historyNicks, historyPunish, historyBlacklist, licences, temporaryVars, hp, eat) VALUES " \
                       f"('{password}', " \
                       f"'{HASH_GEN}', " \
                       f"'{username}', " \
                       f"'{Temporary[0]}', " \
